...
def runner(request):
    if request.POST and request.POST.get("tooltip"):
        desc = Functions.objects.filter(name=request.POST.get("tooltip")).values_list(
    ...
    # Function list.
    funct_list = (
        Functions.objects.filter(type="runner")
        .values_list("name", flat=True)
        .order_by("name")
    )
...
def wheel(request):
    ...
    # Function list.
    funct_list = (
        Functions.objects.filter(type="wheel")
        .values_list("name", flat=True)
        .order_by("name")
    )
    ...
    return render(
        request, "wheel.html", {"funct_list": funct_list, "minion_list": minion_list}
    )
