...
url = 'http://{host}:{port}'.format(host=os.environ.get('SALT_HOST'),
                                    port=os.environ.get('SALT_PORT'))
...
def get_keys(refresh=False):
    if refresh:
        ...
        minion_status = {'minions_rejected': 'rejected',
        ...
        with api_connect() as api:
            ...
            for key, value in minion_status.items():
                for minion in api_ret[key]:
                    finger_ret = api.wheel('key.finger',
                                           match=minion,
                                           hash_type='sha256')
    ...
    with api_connect() as api:
        ...
        if grain.get(minion_id):
            ...
            Minions.objects.update_or_create(
                minion_id=minion_id,
                defaults={'grain': json.dumps(grain[minion_id]),
                          'pillar': json.dumps(pillar[minion_id])}
            )
            minion_fields = MinionsCustomFields.objects.values('name',
                                                               'function').distinct()
...
def init_db(target):
    with api_connect() as api:
        ...
        for func in modules_func:
            ...
            Functions.objects.update_or_create(name=func,
                                               type='modules',
                                               description=desc or '')
        # Runner.
        # TODO: Factorize.
        ...
        for fun, doc in wheel_docs.items():
            Functions.objects.update_or_create(name=fun,
                                               type='wheel',
                                               description=doc or '')
