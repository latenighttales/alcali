...
def test_jobs_filter_minion(admin_client, jid, highstate, dummy_jid, dummy_state):
    highstate()
    response = admin_client.post(reverse('job_list'), {'limit': 100, 'minion': 'master'})
    assert response.status_code == 200
    assert len(response.json()['data']) == 1
...
def test_job_details(admin_client, jid, highstate):
    highstate()
    response = admin_client.get(reverse('job_detail',
                                        kwargs={'jid': '20190429180928455927',
                                                'minion_id': '2e220fd40bc5'}))
    assert response.status_code == 200


def test_minion_delete(admin_client, minion):
    response = admin_client.post(reverse('minions'), {'minion': '2e220fd40bc5',
                                                      'action': 'delete'})
    assert response.status_code == 200
...
def test_minion_detail(admin_client, minion_master):
    response = admin_client.get(reverse('minion_detail',
                                        kwargs={'minion_id': 'master'}))
    assert response.status_code == 200


def test_minion_detail_graph(admin_client, highstate, dummy_state, minion_master):
    highstate()
    response = admin_client.post(reverse('minion_detail',
                                         kwargs={'minion_id': 'master'}), {'period': 0})
    assert response.status_code == 200
