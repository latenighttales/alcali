    ...
    data() {
      return {
        ...
        headers: [
          ...
          { text: this.$t('components.JobTemplatesTable.Client'), value: "client" },
          { text: this.$t('components.JobTemplatesTable.TargetType'), value: "tgt_type" },
          { text: this.$t('components.JobTemplatesTable.Target'), value: "tgt" },
          { text: this.$t('components.JobTemplatesTable.Function'), value: "fun" },
          { text: this.$t('components.JobTemplatesTable.Arguments'), value: "arg" },
          { text: this.$t('components.JobTemplatesTable.KeywordArguments'), value: "kwarg" },
          { text: this.$t('components.JobTemplatesTable.Batch'), value: "batch" },
          { text: this.$t('components.JobTemplatesTable.Actions'), value: "action", sortable: false },
        ],
        loading: true,
        job_templates: [],
      }
    ...
    methods: {
      ...
      deleteTemplate(id) {
        this.$http.delete(`api/job_templates/${id}/`).then(response => {
          this.$toast(this.$i18n.t("components.JobTemplatesTable.TemplateDeleted"))
        }).then(() => {
          this.loadData()
        })
      },
